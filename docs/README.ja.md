...
- ルートをクリックするとコントローラファイルを開き、対象メソッドの行へ移動
  ...
  - クロージャルートは Laravel 12.55 以降なら `route:list` が返す定義位置へ正確に移動。それより前のバージョンでは `routes/` 配下をルート名・URI で検索（ベストエフォート）
...
5. ビュータイトルのボタン
   - パスで絞り込み: `admin` や `users/{user}` のように URI の一部を入力。空にすると解除
   - ミドルウェアで絞り込み: ミドルウェアを選んで絞り込み
   - 絞り込み中は一覧上部に条件が表示され、両方を解除するクリアボタンが現れる
...
- Laravel 12.55 より前のバージョンではクロージャルートの定義位置が `route:list` に含まれないため、ファイル検索による推測です。`Route::prefix()` などで URI が組み立てられていると見つからないことがあります
